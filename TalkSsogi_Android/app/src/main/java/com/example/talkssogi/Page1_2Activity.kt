...
class Page1_2Activity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        btnSignUp.setOnClickListener {
            ...
            if (newID.isNotEmpty()) {
                Log.d("login", "Before checkUserIdExists API call")
                viewModel.checkUserIdExists(newID).observe(this, Observer { exists ->
                    Log.d("login", "checkUserIdExists API call completed: exists = $exists")
                    if (!exists) {
                        Log.d("login", "User ID does not exist, attempting to register")
                        viewModel.registerUserId(newID) { success ->
                            Log.d("login", "registerUserId API call completed: success = $success")
                            if (success) {
                                Log.d("login", "User ID registered successfully")
                                Toast.makeText(this, "회원가입 성공", Toast.LENGTH_SHORT).show()
                                viewModel.saveUserIdToSharedPreferences(newID)
                                viewModel.navigateToNextPage(this, Page2Activity::class.java)
                            } else {
                                Log.e("login", "Failed to register User ID")
                                Toast.makeText(this, "회원가입 실패", Toast.LENGTH_SHORT).show()
                            }
                        }
                    } else {
                        Log.d("login", "User ID already exists")
                        Toast.makeText(this, "아이디가 이미 사용 중입니다.", Toast.LENGTH_SHORT).show()
                    }
                })
            } else {
                Log.w("login", "ID field is empty")
